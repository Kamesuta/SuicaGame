...
export class GameRoom extends Room<GameState> {
    ...
    override onCreate(options: ExpectedCreateOptions): void | Promise<any> {
        ...
        this.onMessage("updateScore", (client, newScore: number) => {
            const newState = this.state.clone(); // Clone state to avoid mutation issues
            const player = newState.players.get(client.sessionId);
            if (player) {
                player.score = newScore; // Update player score
                console.log(`Player ${player.username} updated score to ${newScore}`);
                this.setState(newState); // Broadcast the updated state
            }
        });
    }
    ...
    override onJoin(client: Client, options?: ExpectedJoinOptions): void | Promise<any> {
        ...
        if (typeof options?.userId != "string"
            || typeof options?.username != "string") return client.leave();

        console.log(`Client ${options.username} (${options.userId}) joined to room with instance id: ${this.roomId}`);
